<template>
  <main class="bg-gradient-to-b from-ink to-midnight text-white">
    <section class="relative min-h-[62vh] overflow-hidden">
      ...
      <div class="relative z-10 mx-auto flex min-h-[62vh] w-full max-w-screen-lg flex-col justify-start px-4 pt-4 pb-4 sm:px-6 sm:pt-5 sm:pb-6">
        ...
        <div class="mt-4 sm:mt-6">
          ...
          <div class="mt-2 grid grid-cols-3 gap-2 sm:mt-3 sm:place-items-center sm:gap-3 md:gap-4">
            <button
              v-for="item in visibleStoryboardItems"
              :key="item.id"
              type="button"
              ...
              :class="{
                'story-card--seating': item.id === 'seating',
                'story-card--contain': isContained(item.id)
              }"
              :style="isContained(item.id) && item.image ? { '--story-image': `url(${item.image})` } : undefined"
            ...
            >
              <NuxtImg
                ...
                :src="item.image"
                alt=""
                class="story-card__media absolute inset-0 h-full w-full"
                width="240"
                :height="isContained(item.id) ? 240 : 300"
                :preset="isContained(item.id) ? 'storyContain' : 'story'"
                loading="lazy"
                decoding="async"
              />
...
const visibleStoryboardItems = computed(() => storyboardItems.value.filter((item) => item.id !== 'photos'))

const containIds = new Set(['message', 'notes', 'thanks'])

function isContained(id?: string) {
  return id ? containIds.has(id) : false
}
